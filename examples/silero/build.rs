...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match &config.model {
        config::ModelSource::Local { files } => {
            ...
            for file_spec in files {
                ...
                if !path.exists() {
                    ...
                    generate_stub(class_name, output_dir, &format!("Model file not found: {}", file_spec.path))?;
                    return Ok(());
                }
            }
        }
        config::ModelSource::HuggingFaceHub { repo, revision, files } => {
            ...
            for file_spec in files {
                let dest_name = file_spec.dest.as_ref()
                    .map(|s| s.as_str())
    ...
    if !model_path.exists() {
        ...
        println!("cargo:warning=Generating stub implementation");
        generate_stub(class_name, output_dir, &format!("Model file not found: {}", model_file))?;
        return Ok(());
    }
    ...
    match generate_model_code(
        ...
        Err(e) => {
            ...
            println!("cargo:warning=Generating stub implementation");
            generate_stub(class_name, output_dir, &format!("Code generation failed: {}", e))?;
        }
    }
